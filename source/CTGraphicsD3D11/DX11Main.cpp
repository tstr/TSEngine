	...
	namespace DX11
	{
		...
		*/
		...
		public:
				...
				) override
			{
				if (type == ShaderType::SHADER_TYPE_VERTEX)
				{
					...
					CT_ASSERT(SUCCEEDED(m_device->CreateVertexShader(bytecode, bytecodesize, 0, &vshader)));
				...
				else if (type == ShaderType::SHADER_TYPE_PIXEL)
				{
					...
					CT_ASSERT(SUCCEEDED(m_device->CreatePixelShader(bytecode, bytecodesize, 0, &pshader)));
				...
				else if (type == ShaderType::SHADER_TYPE_GEOMETRY)
				{
					...
					CT_ASSERT(SUCCEEDED(m_device->CreateGeometryShader(bytecode, bytecodesize, 0, &gshader)));
				...
				) override
			{
				...
				CT_ASSERT(SUCCEEDED(m_device->CreateInputLayout(&desc[0], sid_num, bytecode, bytecodesize, &inputlayout)));
				...
				CT_ASSERT(SUCCEEDED(m_device->CreateInputLayout(layout, ARRAYSIZE(layout), bytecode, bytecodesize, m_inputlayout.GetAddressOf())));
				...
				) override
			{
				...
				CT_ASSERT(fmtSupport & D3D11_FORMAT_SUPPORT_MIP_AUTOGEN);
			...
			{
				CT_ASSERT(m_renderTargetDefault.ResizeView(w, h));
		...
		DX11RenderContext::DX11RenderContext(DX11Render* renderer) :
			m_renderer(renderer)
		{
			CT_ASSERT(SUCCEEDED(m_renderer->GetDevice()->CreateDeferredContext(0, m_context.GetAddressOf())));
